 ...
 * Copyright(c) 2008-2010, Thorsten Suckow-Homberg <ts@siteartwork.de>.
 * 
...
{var cm=this._view.cm,index=cm.getIndexById(item.itemId.substr(9)),dIndex=cm.getDataIndex(index);if(index!=-1){if(item.checked){this._view.enableGrouping=false;this._grid.store.clearGrouping();}else{this._view.enableGrouping=true;this._grid.store.groupBy(dIndex);this._lastGroupField=this._view.getGroupField();}}},_getMenuButton:function()
