...
/**
 * Default listener for the mail grid's events
 *
 * @author Thorsten Suckow-Homberg <tsuckow@conjoon.org>
 */
Ext.defineClass('conjoon.mail.grid.comp.listener.GridListener', {
    ...
    constructor : function(config) {
        ...
        if (!(config.gridController instanceof
            conjoon.mail.grid.comp.controller.GridController)) {
            throw new cudgets.base.InvalidPropertyException(
                "gridController not instance of conjoon.mail.grid."+
                "comp.controller.GridController"
            );
        }

        if (!(config.grid instanceof Ext.ux.grid.livegrid.GridPanel)) {
            throw new cudgets.base.InvalidPropertyException(
    ...
    installEvents : function(install){
        ...
        if (install !== false) {
            me.grid.on('resize', me.onGridResize, me);
        } else {
            me.grid.un('resize', me.onGridResize, me);
        }
...
// listeners --------
    ...
    /**
     * Listener for the grid's resize event
     */
    onGridResize : function()
    {
        ...
        gridController.centerErrorMessageComp();
    },
